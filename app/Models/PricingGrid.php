...
class PricingGrid extends Model {
    ...
    public static function setPriceByPriceCode($price_code, $price_value, $product_id)
    {
        ...
        $column_price = \DB::table(\App\Helpers\Project::PRICES_TABLE_NAME)->where('product_id', $product_id)->where('column_id', $column_id)->first();

        if ($column_price) {
            \DB::table(\App\Helpers\Project::PRICES_TABLE_NAME)
